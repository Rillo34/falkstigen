...
def sort_key(filename): # Tar allt före första '-' och gör om till int 
    num = filename.split('-', 1)[0] 
    return int(num) 

image_files = sorted(
    (f.name for f in image_folder.iterdir() if '-' in f.name),
    key=sort_key
)
...
def create():
    ...
    with ui.grid().classes('grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4'): 
        for filename in image_files: # Exempel: "1-fasad.jpg" → "fasad" 
            with ui.card(): 
